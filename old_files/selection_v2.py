...
## Distinction between muon and electron leptonic decay of W is made 
...
gInterpreter.Declare("""
      ...
      auto muonInd(UInt_t nmu, Vfloat pt, Vfloat eta, Vfloat iso) {
		...
		if (pt[i]>30. && fabs(eta[i])<2.4 && iso[i]<0.20){
                	vb.push_back(i);
		}
            }
            return vb;
      ...
      auto JetInd(UInt_t njet, Vfloat pt, Vfloat eta, Vfloat phi, Vint mu_good, Vfloat mu_eta, Vfloat mu_phi, Vint el_good, Vfloat el_eta, Vfloat el_phi) {
		...
		if(mu_good.size()>0){
			cond = ROOT::VecOps::DeltaR(mu_eta[mu_good[0]],eta[i],mu_phi[mu_good[0]],phi[i]) > 0.4;
                        if (pt[i]>30. && fabs(eta[i])<2.4 && cond){
                                vb.push_back(i);
                        }
		}
                if(el_good.size()>0){
...
if "WW" in samples:
	...
	df["WW_hadronic"] = df["WW"].Filter('typeWW == 1')
	df["WW_leptonic"] = df["WW"].Filter('typeWW == 3')
	df["WW_semi_charm"] = df["WW"].Filter('typeWW == 2 && typeC == 1')
	df["WW_semi_nocharm"] = df["WW"].Filter('typeWW == 2 && typeC != 1')
	## Samples correction
	samples.remove("WW")
	samples.append("WW_hadronic")
	samples.append("WW_leptonic")
	samples.append("WW_semi_charm")
	samples.append("WW_semi_nocharm")
...
for s in samples:
	hist_nJetGood_M[s] = df_muon[s].Histo1D(("nJetGood M","",10,0,10),"nJetGood")
	hist_nJetGood_E[s] = df_electron[s].Histo1D(("nJetGood E","",10,0,10),"nJetGood")

	hist_jet_muon_pt_M[s] = df_muon[s].Histo1D(("jet muon pt M","",100,0,500),"jet_muon_pt")
	hist_jet_notmuon_pt_M[s] = df_muon[s].Histo1D(("jet not muon pt M","",100,0,500),"jet_notmuon_pt")
	hist_jet_muon_eta_M[s] = df_muon[s].Histo1D(("jet muon eta M","",80,-4,4),"jet_muon_eta")
	hist_jet_notmuon_eta_M[s] = df_muon[s].Histo1D(("jet not muon eta M","",80,-4,4),"jet_notmuon_eta")

	hist_jet_muon_pt_E[s] = df_electron[s].Histo1D(("jet muon pt E","",100,0,500),"jet_muon_pt")
	hist_jet_notmuon_pt_E[s] = df_electron[s].Histo1D(("jet not muon pt E","",100,0,500),"jet_notmuon_pt")
	hist_jet_muon_eta_E[s] = df_electron[s].Histo1D(("jet muon eta E","",80,-4,4),"jet_muon_eta")
	hist_jet_notmuon_eta_E[s] = df_electron[s].Histo1D(("jet not muon eta E","",80,-4,4),"jet_notmuon_eta")

	hist_lepton_pt_M[s] = df_muon[s].Histo1D(("lepton pt M","",100,0,500),"lepton_pt")	
	hist_lepton_eta_M[s] = df_muon[s].Histo1D(("lepton eta M","",80,-4,4),"lepton_eta")

	hist_lepton_pt_E[s] = df_electron[s].Histo1D(("lepton pt E","",100,0,500),"lepton_pt")
	hist_lepton_eta_E[s] = df_electron[s].Histo1D(("lepton eta E","",80,-4,4),"lepton_eta")

	hist_muon_jet_pt_M[s] = df_muon[s].Histo1D(("muon jet pt M","",100,0,50),"muon_jet_pt")
	hist_muon_jet_eta_M[s] = df_muon[s].Histo1D(("muon jet eta M","",80,-4,4),"muon_jet_eta")

	hist_muon_jet_pt_E[s] = df_electron[s].Histo1D(("muon jet pt E","",100,0,50),"muon_jet_pt")
	hist_muon_jet_eta_E[s] = df_electron[s].Histo1D(("muon jet eta E","",80,-4,4),"muon_jet_eta")

	hist_InvM_2jets_M[s] = df_muon[s].Histo1D(("InvM 2jets M","",100,0,500),"InvM_2jets")
	hist_InvM_2jets_E[s] = df_electron[s].Histo1D(("InvM 2jets E","",100,0,500),"InvM_2jets")
	...
	hist_MET_M[s] = df_muon[s].Histo1D(("MET pt M","",100,0,500),"MET_pt")
	hist_MET_E[s] = df_electron[s].Histo1D(("MET pt E","",100,0,500),"MET_pt")

	hist_deltaR_jetM_lepM[s] = df_muon[s].Histo1D(("deltaR jetM lepM","",100,0,5),"deltaR_jetM_lepM")
	hist_deltaR_jetM_lepE[s] = df_electron[s].Histo1D(("deltaR jetM lepE","",100,0,5),"deltaR_jetM_lepE")
	hist_deltaR_jetM_jetNM_M[s] = df_muon[s].Histo1D(("deltaR jetM jetNM M","",100,0,5),"deltaR_jetM_jetNM")
	hist_deltaR_jetM_jetNM_E[s] = df_electron[s].Histo1D(("deltaR jetM jetNM E","",100,0,5),"deltaR_jetM_jetNM")
	...
	hist_tracks_jetM_M[s] = df_muon[s].Histo1D(("tracks jetM M","",60,0,60),"tracks_jetM")
	hist_tracks_jetNM_M[s] = df_muon[s].Histo1D(("tracks jetNM M","",60,0,60),"tracks_jetNM")

	hist_tracks_jetM_E[s] = df_electron[s].Histo1D(("tracks jetM E","",60,0,60),"tracks_jetM")
	hist_tracks_jetNM_E[s] = df_electron[s].Histo1D(("tracks jetNM E","",60,0,60),"tracks_jetNM")

	hist_EMN_jetM_M[s] = df_muon[s].Histo1D(("EMN jetM M","",60,0,1),"EMN_jetM")
	hist_EMC_jetM_M[s] = df_muon[s].Histo1D(("EMC jetM M","",60,0,1),"EMC_jetM")
	hist_EMN_jetM_E[s] = df_electron[s].Histo1D(("EMN jetM E","",60,0,1),"EMN_jetE")
	hist_EMC_jetM_E[s] = df_electron[s].Histo1D(("EMC jetM E","",60,0,1),"EMC_jetE")

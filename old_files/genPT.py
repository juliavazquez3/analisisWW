...
df2 = df1.Define("W_charge",'GenPart_pdgId[partID[0]]*GenPart_pdgId[partID[1]]/576')
df1 = df2.Define("W_hadronic_charge",'fabs(GenPart_pdgId[partID[2]])<9 ? GenPart_pdgId[partID[0]]/fabs(GenPart_pdgId[partID[0]]) : GenPart_pdgId[partID[1]]/fabs(GenPart_pdgId[partID[1]])')
...
df_aux = df4.Define("charm_pt",'fabs(GenPart_pdgId[partID[2]])==4 ?  GenPart_pt[partID[2]] : (fabs(GenPart_pdgId[partID[3]])==4 ?  GenPart_pt[partID[3]] : (fabs(GenPart_pdgId[partID[4]])==4 ?  GenPart_pt[partID[4]] : (fabs(GenPart_pdgId[partID[5]])==4 ?  GenPart_pt[partID[5]] :-20)))')
...
df_aux = df_aux.Define('aux_charm_status','charmHard(nGenPart,GenPart_statusFlags,GenPart_pdgId)')
df_aux = df_aux.Define('charm_quantity','aux_charm_status[0]')
df_aux = df_aux.Define('charm_status_quantity','aux_charm_status[1]')
df_aux = df_aux.Define('charm_index','charmIndex(nGenPart,GenPart_statusFlags,GenPart_pdgId)')

df_aux = df_aux.Define('ishard','vectorHP(nGenPart,GenPart_statusFlags,GenPart_pdgId,8)')
df_aux = df_aux.Define('fromhard','vectorHP(nGenPart,GenPart_statusFlags,GenPart_pdgId,9)')

df_aux = df_aux.Define('nMuon_gen','LeptonIndex(nGenPart,GenPart_statusFlags,GenPart_pdgId,13).size()')
df_aux = df_aux.Define('nElectron_gen','LeptonIndex(nGenPart,GenPart_statusFlags,GenPart_pdgId,11).size()')

df_aux = df_aux.Define('nMuonGood_gen','LeptonIndexGood(nGenPart,GenPart_statusFlags,GenPart_pdgId,13, GenPart_pt, GenPart_eta).size()')
df_aux = df_aux.Define('nElectronGood_gen','LeptonIndexGood(nGenPart,GenPart_statusFlags,GenPart_pdgId,11, GenPart_pt, GenPart_eta).size()')
...
Nocharm = df4.Filter('typeC == -1')

#print("%s charm entries have at least 1 good jet" %charm.Count().GetValue())
#print("%s not charm entries have at least 1 good jet" %Nocharm.Count().GetValue())
#print("%s charm entries have at least 1 good charm jet" %charm.Filter('nCharmGenJets > 0').Count().GetValue())
#print("%s not charm entries have at least 1 good charm jet" %Nocharm.Filter('nCharmGenJets > 0').Count().GetValue())

#print("%s charm entries have at least 1 good jet associated to a charm gen particle" %charm.Filter('CharmJetPartFlavour < 6').Count().GetValue())
#print("%s charm entries have at least 1 good charm jet associated to a charm gen particle" %charm.Filter('fabs(CharmJetPartFlavour) == 4').Count().GetValue())

#print("%s not charm entries have at least 1 good jet associated to a charm gen particle" %Nocharm.Filter('CharmJetPartFlavour < 6').Count().GetValue())
#print("%s not charm entries have at least 1 good charm jet associated to a charm gen particle" %Nocharm.Filter('fabs(CharmJetPartFlavour) == 4').Count().GetValue())
...
gInterpreter.Declare("""
	...
	auto indMuonJet(UInt_t nGen, Vint pdg, Vint mother, Vfloat part_pt, Vfloat part_eta, Vfloat part_phi, Vfloat jet_eta, Vfloat jet_phi, Vint GoodJets) {
		int ind = -1;
			...
			auto cond1 = ROOT::VecOps::DeltaR(part_eta[i],jet_eta[GoodJets[j]] , part_phi[i], jet_phi[GoodJets[j]])<0.4;
...
gInterpreter.Declare("""
      ...
      auto rawCharmJets(UInt_t nGenJ,Vint flavour){
            int ind = 0;
            for (unsigned int i=0; i<nGenJ; ++i) {
                if (fabs(flavour[i])==4) {
                       ind++;
      ...
      auto PTrawCharmJets(UInt_t nGenJ,Vint flavour, Vfloat pt){
            float pT{-10.};
            for (unsigned int i=0; i<nGenJ; ++i) {
      ...
      auto semiCharmJets(UInt_t nGenJ,Vint flavour, Vfloat pt, Vfloat eta){
            int ind = 0;
            for (unsigned int i=0; i<nGenJ; ++i) {
                if (fabs(flavour[i])==4 && pt[i]>30 && fabs(eta[i])<2.5) {
                       ind++;
      ...
      auto rawMuonJet(UInt_t nGenJ,Vint flavour, Vfloat pt, Vfloat eta, Vfloat phi, UInt_t nPart, Vfloat part_pt, Vfloat part_eta, Vfloat part_phi, Vint pdg){
            int indJ = -1;
	    float pT{-10.};
	    int ind = 0;
            for (unsigned int i=0; i<nGenJ; ++i) {
	    ...
	    if (indJ != -1){
            	for (unsigned int j=0; j<nPart; ++j) {
                	if ( fabs(pdg[j]) == 13 && ROOT::VecOps::DeltaR(part_eta[j], eta[indJ] , part_phi[j], phi[indJ])<0.25) {
                        	ind++;
                	}
            	}
	    }
      ...
      auto semiMuonJet(UInt_t nGenJ,Vint flavour, Vfloat pt, Vfloat eta, Vfloat phi, UInt_t nPart, Vfloat part_pt, Vfloat part_eta, Vfloat part_phi, Vint pdg){
            int indJ = -1;
            float pT{-10.};
            int ind = 0;
            for (unsigned int i=0; i<nGenJ; ++i) {
                if (fabs(flavour[i])==4 && pt[i]>pT && pt[i]>30 && fabs(eta[i])<2.5) {
                        indJ = i;
                        pT = pt[i];
                }
            }
      ...
      auto PtMuonJet(UInt_t nGenJ,Vint flavour, Vfloat pt, Vfloat eta, Vfloat phi, UInt_t nPart, Vfloat part_pt, Vfloat part_eta, Vfloat part_phi, Vint pdg){
            int indJ = -1;
            float pT{-10.};
            int ind = 0;
            for (unsigned int i=0; i<nGenJ; ++i) {
                if (fabs(flavour[i])==4 && pt[i]>pT && pt[i]>30 && fabs(eta[i])<4) {
                        indJ = i;
                        pT = pt[i];
                }
            }
	    float pT_mu{-10.};
            if (indJ != -1){
                for (unsigned int j=0; j<nPart; ++j) {
                        if (part_pt[j]>pT_mu && fabs(pdg[j]) == 13 && ROOT::VecOps::DeltaR(part_eta[j], eta[indJ] , part_phi[j], phi[indJ])<0.4) {
                                pT_mu = part_pt[j];
                        }
                }
            }
...
df5 = df4.Define('nRawCharmGenJets','rawCharmJets(nGenJet, GenJet_partonFlavour)')
df6 = df5.Define('nSemiRawCharmGenJets','semiCharmJets(nGenJet, GenJet_partonFlavour, GenJet_pt, GenJet_eta)')
df7 = df6.Define('nRawMuonCharmGenJets','rawMuonJet(nGenJet, GenJet_partonFlavour, GenJet_pt, GenJet_eta, GenJet_phi, nGenPart, GenPart_pt, GenPart_eta, GenPart_phi, GenPart_pdgId)')
df4 = df7.Define('nSemiMuonCharmGenJets','semiMuonJet(nGenJet, GenJet_partonFlavour, GenJet_pt, GenJet_eta, GenJet_phi, nGenPart, GenPart_pt, GenPart_eta, GenPart_phi, GenPart_pdgId)')

#charm_ev = df4.Filter('typeC==1').Count().GetValue()
#charm_genjetcharm = df4.Filter('typeC==1 && nRawCharmGenJets > 0').Count().GetValue()
#charm_genjetcharm_filter = df4.Filter('typeC==1 && nSemiRawCharmGenJets > 0').Count().GetValue()
#charm_muon_genjetcharm = df4.Filter('typeC==1 && nRawMuonCharmGenJets > 0').Count().GetValue()
#charm_muon_genjetcharm_filter = df4.Filter('typeC==1 && nSemiMuonCharmGenJets > 0').Count().GetValue()

#q1 = charm_genjetcharm/charm_ev
#q2 = charm_genjetcharm_filter/charm_genjetcharm
#q3 = charm_muon_genjetcharm/charm_genjetcharm
#q4 = charm_muon_genjetcharm_filter/charm_genjetcharm_filter

#print("quantity of charm events is %s" %charm_ev)
#print("proportion of charm events with charm gen jet is %s " %q1)
#print("proportion of charm jets with proper pt and eta is %s " %q2)
#print("proportion of charm jets with muons is %s " %q3)
#print("proportion of proper charm jets with muons is %s " %q4)

df5 = df4.Define('SemiMuonJet_pt','PtMuonJet(nGenJet, GenJet_partonFlavour, GenJet_pt, GenJet_eta, GenJet_phi, nGenPart, GenPart_pt, GenPart_eta, GenPart_phi, GenPart_pdgId)')
df6 = df5.Define('RawCharmJet_pt','PTrawCharmJets(nGenJet, GenJet_partonFlavour, GenJet_pt)')
df4 = df6.Define("charm_pt_1",'fabs(GenPart_pdgId[partID[2]])==4 ?  GenPart_pt[partID[2]] : (fabs(GenPart_pdgId[partID[3]])==4 ?  GenPart_pt[partID[3]] : (fabs(GenPart_pdgId[partID[4]])==4 ?  GenPart_pt[partID[4]] : (fabs(GenPart_pdgId[partID[5]])==4 ?  GenPart_pt[partID[5]] :-20)))')
...
#hmet1 = df4.Histo1D("W_charge")
#plot([hmet1],["Semi"],"/W_charge.pdf",logY=True)

#hmet1 = charm.Histo1D(("","",10,0,10),"charm_quantity")
#hmet2 = notCharm.Histo1D(("","",10,0,10),"charm_quantity")
#hmet3 = charm.Histo1D(("","",10,0,10),"charm_status_quantity")
#hmet4 = notCharm.Histo1D(("","",10,0,10),"charm_status_quantity")
#plot([hmet4,hmet1,hmet2,hmet3],["No Charm HardP","Charm","Not charm","Charm HardP"],"/charm_quantity.pdf")

#hmet1 = charm.Histo1D(("","",100,0,500),"W_hadronic_pt")
#hmet2 = notCharm.Histo1D(("","",100,0,500),"W_hadronic_pt")
#hmet3 = charm.Histo1D(("","",100,0,500),"W_leptonic_pt")
#hmet4 = notCharm.Histo1D(("","",100,0,500),"W_leptonic_pt")
#plot([hmet1,hmet2,hmet3,hmet4],["Hadronic W from charm","Hadronic W from not charm","Leptonic W from charm","Leptonic W from not charm"],"/genWW_pt.pdf",logY=True)
...
#hmet1 = charm.Histo1D(("","",100,0,50),"SemiMuonJet_pt")
#plot([hmet1],["Charm"],"/GenMuonJet_pt_050.pdf",logY=True)

#hmet1 = charm.Histo1D(("","",100,50,250),"SemiMuonJet_pt")
#plot([hmet1],["Charm"],"/GenMuonJet_pt_50250.pdf",logY=True)
...
########## charm jets vs charm particles

#c1 = ROOT.TCanvas('c1','c1',800,700)

#h2 = df4.Histo2D(("h2", "scatter plot", 100,0 , 400, 100, 0, 400),"charm_pt_1","RawCharmJet_pt")
#h2.Draw("COLZ")
#h2.SetStats(0);
#h2.GetXaxis().SetTitle( 'Charm Particle pT' )
#h2.GetYaxis().SetTitle( 'Charm Jet pT' )
#c1.SaveAs(plotdir+"scatterCharmPTpartVSjet.pdf")
...
df4.Snapshot("Events", "SemiGen.root",brlist2)

...
## Selection for MC samples, created to distinguish between years
...
if args.process == "allMC": proc = ["WW","Wjets1","DY1","Wjets2","DY2","Wjets3","DY3","Wjets4","DY4","Wjets5","DY5","Wjets6","DY6","Wjets7","DY7","Wjets8","DY8",
	...
	or args.process == "ttbarhad"): proc = [str(args.process)]
...
for s in samples:
	if (s[0]+s[1] == "WW" and (s[-1]=="6" or s[-1]=="7" or s[-1]=="8")):
...
for s in samples:
        if (s[0]+s[1]+s[2]+s[3]+s[4] == "ttbar" and (s[-1]=="6" or s[-1]=="7" or s[-1]=="8")):
...
for s in samples:
	...
	hist_tranverse_massM[s] = df_muon[s].Histo1D(("transverse massM","",50,0,150),"transverse_mass","weightSSOS")
	hist_tranverse_massE[s] = df_electron[s].Histo1D(("tranverse massE","",50,0,150),"transverse_mass","weightSSOS")

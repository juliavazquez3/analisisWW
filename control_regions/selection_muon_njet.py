...
for s in samples:
	df[s] = df[s].Filter('nMuonGood<2 && nElectronGood<2').Filter('!(nMuonGood==1) != !(nElectronGood==1)').Filter('MuonJetInd.size() >= 1').Filter('MuonJetGood')
	### hists definitions
...
if args.notfull:
	for s in samples:
		if args.ssos : path_hist = '/nfs/cms/vazqueze/analisisWW/hists/controlR/ssos/njet_v1v2v3v4SSOS_'+s+'_range_'+str(args.list[0])+'_'+str(args.list[1])+'.root'
		else: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/controlR/njet_v1v2v3v4_'+s+'_range_'+str(args.list[0])+'_'+str(args.list[1])+'.root'
	...
	for s in samples:
                if args.ssos: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/controlR/ssos/njet_v1v2v3v4SSOS_'+s+'.root'
                else: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/controlR/njet_v1v2v3v4_'+s+'.root'
                myfile = TFile( path_hist, 'RECREATE' )

...
for s in samples:
	df[s] = df[s].Filter('nMuonGood<2 && nElectronGood<2').Filter('!(nMuonGood==1) != !(nElectronGood==1)').Filter('nJetGood==2').Filter('SVJetInd.size() >= 1').Filter('SVJetGood')
	df[s] = df[s].Filter('!(MuonJetInd.size() >= 1 && MuonJetGood)')
	df[s] = df[s].Define('JetnotSVInd','nJetGood>1 ? (JetSVInd[0] == JetGoodInd[0] ? JetGoodInd[1] : JetGoodInd[0]) : -1')
	...
	df[s] = df[s].Define('InvM_2jets','nJetGood>1 ? InvariantM(Jet_pt[JetSVInd[0]],Jet_eta[JetSVInd[0]],Jet_phi[JetSVInd[0]],Jet_mass[JetSVInd[0]],Jet_pt[JetnotSVInd],Jet_eta[JetnotSVInd],Jet_phi[JetnotSVInd],Jet_mass[JetnotSVInd]) : 0')
...
for s in samples:
	...
	df_muon[s] = df_muon[s].Define('deltaR_jetM_lepM','ROOT::VecOps::DeltaR(Muon_eta[MuonGoodInd[0]],Jet_eta[JetSVInd[0]] , Muon_phi[MuonGoodInd[0]], Jet_phi[JetSVInd[0]])')
	df_electron[s] = df_electron[s].Define('deltaR_jetM_lepE','ROOT::VecOps::DeltaR(Electron_eta[ElectronGoodInd[0]],Jet_eta[JetSVInd[0]] , Electron_phi[ElectronGoodInd[0]], Jet_phi[JetSVInd[0]])')
	...
	#df_electron[s] = df_electron[s].Filter('pT_Wlep>110.')
	#df_muon[s] = df_muon[s].Filter('pT_Wlep>110.')
...
for s in samples:
	...
	hist_jet_notmuon_pt_M[s] = df_muon[s].Histo1D(("jet_not_sv_pt_M","",50,20,120),"jet_notmuon_pt","weightSSOS")
	hist_jet_muon_eta_M[s] = df_muon[s].Histo1D(("jet_sv_eta_M","",80,-4,4),"jet_muon_eta","weightSSOS")
	hist_jet_notmuon_eta_M[s] = df_muon[s].Histo1D(("jet_not_sv_eta_M","",80,-4,4),"jet_notmuon_eta","weightSSOS")
	hist_jet_notmuon_nmu_M[s] = df_muon[s].Histo1D(("jet_notsv_nmu_M","",10,0,10),"jet_notmuon_nmu","weightSSOS")
	hist_jet_notmuon_mass_M[s] = df_muon[s].Histo1D(("jet_notsv_mass_M","",40,0,40),"jet_notmuon_mass","weightSSOS")
	hist_jet_notmuon_qgl_M[s] = df_muon[s].Histo1D(("jet_notsv_qgl_M","",100,0,1),"jet_notmuon_qgl","weightSSOS")
	...
	hist_jet_notmuon_pt_E[s] = df_electron[s].Histo1D(("jet_not_sv_pt_E","",50,20,120),"jet_notmuon_pt","weightSSOS")
	hist_jet_muon_eta_E[s] = df_electron[s].Histo1D(("jet_sv_eta_E","",80,-4,4),"jet_muon_eta","weightSSOS")
	hist_jet_notmuon_eta_E[s] = df_electron[s].Histo1D(("jet_not_sv_eta_E","",80,-4,4),"jet_notmuon_eta","weightSSOS")
	hist_jet_notmuon_nmu_E[s] = df_electron[s].Histo1D(("jet_notsv_nmu_E","",10,0,10),"jet_notmuon_nmu","weightSSOS")
	hist_jet_notmuon_mass_E[s] = df_electron[s].Histo1D(("jet_notsv_mass_E","",40,0,40),"jet_notmuon_mass","weightSSOS")
	hist_jet_notmuon_qgl_E[s] = df_electron[s].Histo1D(("jet_notsv_qgl_E","",100,0,1),"jet_notmuon_qgl","weightSSOS")
	...
	hist_jet_notmuon_btag_M[s] = df_muon[s].Histo1D(("jet_notsv_btag_M","",50,0,1),"jet_notmuon_btag","weightSSOS")
	hist_jet_notmuon_btag_E[s] = df_electron[s].Histo1D(("jet_notsv_btag_E","",50,0,1),"jet_notmuon_btag","weightSSOS")
	...
	hist_pT_sum_2J_M[s] = df_muon[s].Histo1D(("pT_sum 2J_M","",100,0,1),"pT_sum_2J","weightSSOS")
	hist_pT_sum_2J_E[s] = df_electron[s].Histo1D(("pT_sum 2J_E","",100,0,1),"pT_sum_2J","weightSSOS")
...
if args.notfull:
	for s in samples:
		if args.ssos: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/SV/ssos/sv_v1v3v4v5SSOS_'+s+'_range_'+str(args.list[0])+'_'+str(args.list[1])+'.root'
		else: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/SV/sv_v1v3v4v5_'+s+'_range_'+str(args.list[0])+'_'+str(args.list[1])+'.root'
	...
	for s in samples:
                if args.ssos: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/SV/ssos/sv_v1v3v4v5SSOS_'+s+'.root'
                else: path_hist = '/nfs/cms/vazqueze/analisisWW/hists/SV/sv_v1v3v4v5_'+s+'.root'
                myfile = TFile( path_hist, 'RECREATE' )

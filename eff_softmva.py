...
for name in ["muon_jet_mva_M","muon_jet_mva_E","nJetGood_E","nJetGood_M"]:
  ...
  if (name=="nJetGood_M" or name=="nJetGood_E"):
    hist_signal = histT[samples[0]]
    hist_backg = histT[samples[1]]
    for s in samples[2:]:
      hist_backg.Add(histT[s])
    Qsig = hist_signal.Integral()
    Qbak = hist_backg.Integral()
    print("With soft mva cut !!")
    print("Number of MC signal events for "+name[-1]+" channel is "+str(Qsig))
    print("Number of MC background events for "+name[-1]+" channel is "+str(Qbak))
    print("Ratio signal over background is "+str(Qsig/Qbak))
    print("Significance is "+str(Qsig/sqrt(Qbak)))

  if (name=="muon_jet_tight_M" or name=="muon_jet_tight_E"):
    hist_signal = histT[samples[0]]
    hist_backg = histT[samples[1]]
    for s in samples[2:]:
      hist_backg.Add(histT[s])
